...
def run_parallel(
    ...
    # Execution Context Manager (Live UI or Null)
    from contextlib import nullcontext
    ui_context = Live(make_layout(), refresh_per_second=4) if not quiet else nullcontext()

    with ui_context as live:
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                if progress_callback:
                    progress_callback(int((completed_count / len(chunks)) * 100))
...
Examples:
  python render_engine_v2.py input.mp4 --smart --sweep
    ...
    # Setup
    max_workers_arg = args.workers or psutil.cpu_count(logical=True)
    filter_chain_arg = args.filter
    ...
    try:
        ...
        if not args.skip_serial:
            ...
            serial_time = run_serial_baseline(args.input, serial_output, filter_chain_arg)
        ...
        if args.sweep:
            sweep_results = run_scaling_sweep(
                args.input, metadata, filter_chain_arg, max_workers_arg, str(temp_dir)
            )
            benchmark_data["sweep_results"] = sweep_results
            # Copy best result to output
            best_output = temp_dir / f"output_{max_workers_arg}w.mp4"
            if best_output.exists():
                shutil.copy(best_output, args.output)
        else:
            console.print(f"\n[bold]Running parallel processing ({max_workers_arg} workers)...[/bold]")
            elapsed, results = run_parallel(
                args.input, args.output, metadata,
                max_workers_arg, filter_chain_arg, str(temp_dir)
            )
            sweep_results = {max_workers_arg: {'time': elapsed, 'success': True}}
            benchmark_data["sweep_results"] = sweep_results

        # Display results
        if serial_time:
            display_results(metadata, serial_time, sweep_results, max_workers_arg)
        else:
            console.print(f"\n[green]Done![/green] Output: {args.output}")
            if sweep_results.get(max_workers_arg, {}).get('time'):
                console.print(f"Parallel time: {sweep_results[max_workers_arg]['time']:.2f}s")
