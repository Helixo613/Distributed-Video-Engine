...
export function ChunkVisualizer({ activeJob }: ChunkVisualizerProps) {
  const totalChunks = activeJob
    ? activeJob.planned_chunk_boundaries?.length || activeJob.chunk_count || activeJob.workers * 2
    : 16;

  // Legacy UI fallback: synthesize chunk cells when the API does not expose planned chunks.
  const getChunks = () => {
      ...
      {activeJob && (
        <div className="mt-4 pt-4 border-t border-[#222] flex justify-between items-center">
            <span className="text-xs font-medium text-zinc-400">
                Strategy: <span className="text-white">{activeJob.smart_config ? 'Adaptive Partitioning' : 'Equal-Duration Partitioning'}</span>
            </span>
            <span className="text-xs font-mono text-zinc-500">
                {totalChunks} Chunks / {activeJob.workers} Workers
            </span>
        </div>
      )}
    </div>
