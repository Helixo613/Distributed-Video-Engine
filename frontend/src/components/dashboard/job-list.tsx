          ...
          <div key={job.id} className="p-4 hover:bg-white/[0.02] transition-colors group">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    {job.chunk_count ? (
                      <>
                        <div className="w-1 h-1 rounded-full bg-zinc-700" />
                        <span className="text-[10px] text-zinc-500 font-medium">{job.chunk_count} Chunks</span>
                      </>
                    ) : null}
                    {job.partition_policy ? (
                      <>
                        <div className="w-1 h-1 rounded-full bg-zinc-700" />
                        <span className="text-[10px] text-cyan-400 font-medium uppercase">{job.partition_policy}</span>
                      </>
                    ) : null}
                                    {job.engine_version && (
                                      <>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-4 pl-4 border-l border-[#222222]">
                ...
                {job.performance?.speedup_vs_serial ? (
                  <div className="flex flex-col min-w-[60px]">
                    <span className="text-[10px] text-zinc-500 font-bold uppercase">Speedup</span>
                    <span className="text-xs font-semibold tabular-nums">{Number(job.performance.speedup_vs_serial).toFixed(2)}x</span>
                  </div>
                ) : null}
                {job.output && (
                   <div className="flex flex-col">
                      ...
                      {job.experiment_record_path && (
                        <a href={apiUrl(job.experiment_record_path)} target="_blank" rel="noreferrer" className="text-[10px] text-cyan-400 font-bold uppercase hover:underline cursor-pointer">
                          Record
                        </a>
                      )}
                   </div>
